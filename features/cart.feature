Feature: Cart Management
  ...
  Scenario: Fail to add more than allowed quantity
    When a user adds product "first" with quantity 20 to cart
    And a user adds product "second" with quantity 20 to cart
    And a user adds product "third" with quantity 11 to cart
